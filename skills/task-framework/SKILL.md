...
**`output/deliveries/` 目录** — 唯一的外部交付根。需要发送给用户、合作方或其他外部接收方的文件必须集中在一个 `output/deliveries/<bundle>/` 下；该 bundle 是可压缩、可发送的单一入口，包含最终二进制、文档、附件以及 bundle 根 `SHA256SUMS`。候选库、不同工具链产物等必须作为该 bundle 的子目录，不能在 `output/deliverables/` 或任何平行根目录中分散创建。若重建交付压缩包，保留旧版本并以 `.revN.zip` 递增命名。
